...
class pyTextWidget(plasmascript.Applet):
    ...
    def connectToEngine(self):
        ...
        self.systemmonitor.connectSource("network/interfaces/net0/transmitter/data", self, 200)
        self.systemmonitor.connectSource("network/interfaces/net0/receiver/data", self, 200)
    
    @pyqtSignature("dataUpdated(const QString &, const Plasma::DataEngine::Data &)")
    def dataUpdated(self, sourceName, data):
        if (sourceName == "cpu/system/TotalLoad") and (data[QString(u'value')]):
            value = str(round(float(data[QString(u'value')]), 1))
            self.cpuText = "%5s" % (value)
        if (sourceName == "network/interfaces/net0/transmitter/data") and (data[QString(u'value')]):
            value = str(round(float(data[QString(u'value')]), 0))
            self.up_speed = "%4s" % (value)
        if (sourceName == "network/interfaces/net0/reciever/data") and (data[QString(u'value')]):
            value = str(round(float(data[QString(u'value')]), 0))
